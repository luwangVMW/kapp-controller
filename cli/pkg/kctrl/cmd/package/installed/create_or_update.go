...
func (o *CreateOrUpdateOptions) preparePackageInstallForUpdate(pkgInstall *kcpkgv1alpha1.PackageInstall) (*kcpkgv1alpha1.PackageInstall, error) {
	...
	// If o.PackageName is provided by the user (via --package flag) is not the same as the the package name in PackageInstall
	...
	if o.packageName != "" && updatedPkgInstall.Spec.PackageRef.RefName != o.packageName {
		o.ui.PrintLinef("Update package install '%s' from packageName '%s' to packageName '%s'", o.Name, updatedPkgInstall.Spec.PackageRef.RefName, o.packageName)
